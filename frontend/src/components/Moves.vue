...
const fetchAddresses = async () => {
  ...
  let responseMoves = await fetch(
    "https://polishedcheats-backend.vercel.app/api/moves"
  );
...
<template>
  <Card v-if="moves && addresses">
      ...
      <div class="flex flex-wrap justify-between gap-5">
        ...
        <Button
          v-if="
            ...
            selectedMoves[3]
          "
          @click="copy(getMoveCode(selectedMoves))"
    ...
    <template #content>
      <div
        class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2 mt-2 mb-5"
      >
        <Select
          v-model="selectedMoves[0]"
          :options="moves"
          filter
          placeholder="Select Move 1"
        />
        <Select
          v-model="selectedMoves[1]"
          :options="moves"
          filter
          placeholder="Select Move 2"
        />
        <Select
          v-model="selectedMoves[2]"
          :options="moves"
          filter
          placeholder="Select Move 3"
        />
        <Select
          v-model="selectedMoves[3]"
          :options="moves"
          filter
          placeholder="Select Move 4"
        />
      </div>
      <p
        class="mb-5"
        ...
        "
      >
        Your code for this moveset is: {{ getMoveCode(selectedMoves) }}
      </p>
      <p class="mb-5" v-else>Please choose four moves.</p>
      <p>
        This code modifies the moveset of the first Pokemon in your party.
        <br />
        Note that this allows the Pokemon to learn moves it typically cannot.
        <br />
        Note that upon using this cheat, the PP of the moves will be incorrect,
        since it follows the PP of the old moveset. <br />
        This is easily fixed by healing your Pokemon or putting it into the PC.
        <br />
        Should you wish to change the moveset of the other Pokemon in your
