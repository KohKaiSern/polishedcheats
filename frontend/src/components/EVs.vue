...
<template>
  <Card v-if="addresses">
    <template #title>
      Effort Values
      <Button @click="copy(getEVCode(selectedEVs))" :label="(copied.value ? 'Copied!' : 'Copy')" class="float-right" icon="pi pi-copy" iconPos="right" /></template>
    <template #content>
      ...
      <p class="mb-5">Your code for the Effort Values is: {{ getEVCode(selectedEVs) }}</p>
