...
const getOptionCode = (selectedOptions) => {
  ...
  let shinyValue = ["Non-Shiny", "Shiny"]
    .indexOf(selectedOptions[2])
    .toString(2);
  let abilityValue = ["Hidden Ability", "Ability 1", "Ability 2"]
    .indexOf(selectedOptions[0])
    .toString(2);
  ...
  let cheatValue = parseInt(
    shinyValue + abilityValue + natureValue,
    2
  ).toString(16);
  ...
  <Card v-if="addresses">
    <template #title>
      <div class="flex flex-wrap justify-between gap-5">
        ...
        <Button
          v-if="selectedOptions[0] && selectedOptions[1] && selectedOptions[2]"
          @click="copy(getOptionCode(selectedOptions))"
    ...
    <template #content>
      ...
      >
        <Select
          ...
          :options="['Ability 1', 'Ability 2', 'Hidden Ability']"
        ...
        <Select
          ...
          :options="['Non-Shiny', 'Shiny']"
          placeholder="Choose Shininess"
        />
      </div>
      <p
        class="mb-5"
        v-if="selectedOptions[0] && selectedOptions[1] && selectedOptions[2]"
      >
        Your code for {{ selectedOptions[0] }}, a
        {{ selectedOptions[1] }} Nature and a {{ selectedOptions[2] }} Pokemon
        is: {{ getOptionCode(selectedOptions) }}
      </p>
      <p class="mb-5" v-else>
        Please choose an ability, a nature and a shininess value.
      </p>
      <p>
        This code modifies the ability, nature and shininess of your Pokemon.
        <br />
        If you have abilities and natures turned off in your save settings, this
        cheat will only affect shininess. <br />
        Should you wish to change the ability, nature or shininess of the other
        Pokemon in your party, simply swap the party order to place them first.
      </p>
    </template>
  </Card>
</template>
