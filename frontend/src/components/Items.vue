...
const fetchItems = async () => {
  ...
  let responseItems = await fetch(
    "https://polishedcheats-backend.vercel.app/api/items"
  );
  ...
  <Card v-if="items && addresses">
    <template #title>
      <div class="flex flex-wrap justify-between gap-5">
        ...
        <Button
          v-if="selectedItem"
          @click="copy(getItemCode(selectedItem))"
    ...
    <template #content>
      ...
      />
      <p class="mb-5" v-if="selectedItem">
        Your code for {{ selectedItem }} is: {{ getItemCode(selectedItem) }}
      </p>
      <p class="mb-5" v-else>Please choose an item.</p>
      <p>
        This code replaces the first item in the relevant bag slot with 99 of
        the chosen item. <br />
        Warning! If the relevant bag slot is empty, this code will corrupt your
        save!
      </p>
    </template>
  </Card>
