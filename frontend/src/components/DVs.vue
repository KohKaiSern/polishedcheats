...
const getDVCode = (selectedDVs) => {
  ...
  let addressList = [
    addresses.value["wPartyMon1HPAtkDV"],
    addresses.value["wPartyMon1DefSpeDV"],
    addresses.value["wPartyMon1SatSdfDV"],
  ];
  ...
  <Card v-if="addresses">
    <template #title>
      <div class="flex flex-wrap justify-between gap-5">
        ...
        <Button
          @click="copy(getDVCode(selectedDVs))"
    ...
    <template #content>
      ...
      <p class="mb-5">
        Your code for the Determinant Values is: {{ getDVCode(selectedDVs) }}
      </p>
      <p class="mb-5">
        The Hidden Power Type of your Pokemon will be:
        {{ getHPType(selectedDVs) }}
      </p>
      <p>
        This code modifies the Determinant Values of your Pokemon. As a
        side-effect, this will also affect the type of the Pokemon's Hidden
        Power move.<br />
        If you have Perfect Stats turned on in your save settings, this will
        only affect your Hidden Power Type. <br />
        Should you wish to change the DVs of the other Pokemon in your party,
