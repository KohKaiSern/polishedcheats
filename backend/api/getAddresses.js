...
export const getAddresses = () => {
    ...
    const data = fs.readFileSync(path.join(__dirname, '..', 'public', 'polishedcrystal-3.1.1.sym'), 'utf8')
    ...
    data.trim().split(`\n`).forEach((line) => {   
        ...
        if (SYMBOLS.includes(line.trim().slice(8))) {
            ...
            addresses[line.trim().slice(8)] = line.trim().slice(5, 7) + line.trim().slice(3, 5)
        }
    })
    return addresses;
}
